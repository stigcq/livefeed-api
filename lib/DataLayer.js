...
exports.setAvatar = function(userId, mediaId, callback) {
    ...
    MongoClient.connect(connectString, function (err, client) {
        ...
        var newvalues = { $set: {avatar_url: "Mickey" } };

        db.collection("media").updateOne(myquery, newvalues, function(err, ires) {
            if (err) throw err;
...
exports.addMedia = function(media, callback) {
    ...
    MongoClient.connect(connectString, function (err, client) {
        ...
        db.collection("media").insertOne(media, function(err, ires) {
            if (err) throw err;
            console.log("1 media inserted id " + JSON.stringify(message) );
            client.close();
            callback(message);
        });
